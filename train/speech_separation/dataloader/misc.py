    ...
    with open(input_path) as fid:
        for line in fid:
            ...
            if args.load_type == 'one_input_one_output':            
                ...
                n_lab = getattr(args, 'num_spks', 2)
                # an optional trailing .npy column is the mixture F0 contour,
                # NOT a label; it is used only when the network sets use_f0=1
                has_f0 = len(multi_paths) > 1 + n_lab and multi_paths[-1].endswith('.npy')
                if has_f0:
                    sample = {'inputs': multi_paths[0],
                              'labels': multi_paths[1:1 + n_lab],
                              'f0': multi_paths[-1]}
                else:
                    sample = {'inputs': multi_paths[0],
                              'labels': multi_paths[1:1 + n_lab]}
